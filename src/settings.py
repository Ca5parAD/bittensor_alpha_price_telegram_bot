...
from simple_commands import start_command, handle_unknown_message
...
async def store_subnets(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # ****************** start of theoretical logic ***********************

    # Get rid of spaces in string, therefore e.g. text = "5,7,19,64"
    # subnets = int(text.split(','))
    # set context.user_data['notification_subnets'] to subnets if 0 <= subnet <= 128

    # ****************** end of theoretical logic ***********************
