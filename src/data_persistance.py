...
def initialise_from_database():
    ...
    try:
        ...
        logger.exception(
            f"failed to initialise from database: {e}",
            exc_info=True
        )
    else:
        logger.debug("successfully initialised from database")
...
def search_database_for_user(user_id) -> Union[Dict,bool]:
    ...
    try:
        ...
        logger.exception(
            f"user_id:{user_id} - failed to search database: {e}",
            exc_info=True
        )
    else:
        logger.debug(f"user_id:{user_id} - successfully searched database")
...
def update_database_user_settings(user_id, user_data):
    ...
    try:
        ...
        logger.exception(
            f"user_id:{user_id} - failed to update database: {e}",
            exc_info=True
        )
    else:
        logger.debug(f"user_id:{user_id} - successfully updated database")