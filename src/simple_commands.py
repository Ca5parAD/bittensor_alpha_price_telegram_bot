...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logger.info("user_id:{update.chat.id} - start command")
...
async def show_commands(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logger.debug("user_id:{update.chat.id} - show commands")
    await update.message.reply_text(SHOW_COMMANDS_MESSAGE, parse_mode="HTML")
    return SELECT_COMMAND

async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logger.info("user_id:{update.chat.id} - help command")
    await update.message.reply_text(HELP_MESSAGE, parse_mode="HTML")
    return HELP

async def unknown_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    logger.info("user_id:{update.chat.id} - unknown command")
    await update.message.reply_text(UNKNOWN_COMMAND, parse_mode="HTML")

async def unknown_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    logger.info("user_id:{update.chat.id} - unknown message")
...
async def outside_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    logger.info("user_id:{update.chat.id} - outside conversation")
